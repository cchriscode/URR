...
public class SeatLockService {
    ...
    public SeatLockResult acquireLock(UUID eventId, UUID seatId, String userId) {
        ...
        try {
            ...
            log.warn("Redis seat lock failed for {}, falling back to DB-only: {}", seatKey, ex.getMessage());
            // Graceful degradation: allow DB-only locking
            return new SeatLockResult(true, -1);
        }
    }
    ...
    public boolean verifyForPayment(UUID eventId, UUID seatId, String userId, long token) {
        if (token == -1) {
            // Fallback mode: Redis was unavailable during lock, skip verification
            return true;
        }
        String seatKey = seatKey(eventId, seatId);
        try {
            ...
            log.warn("Redis payment verify failed for {}: {}", seatKey, ex.getMessage());
            // Graceful degradation: allow payment to proceed with DB-only check
            return true;
        }
    }
