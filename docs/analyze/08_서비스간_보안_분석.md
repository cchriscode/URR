...
`urr-common` 공용 라이브러리에 `InternalTokenValidator`가 중앙화되어 있으며, 6개 서비스(catalog, community, payment, queue, stats, ticket)가 공유한다. auth-service는 자체 버전을 유지한다.

**출처**: `urr-common/src/main/java/guru/urr/common/security/InternalTokenValidator.java`

```java
// urr-common/src/main/java/guru/urr/common/security/InternalTokenValidator.java
public class InternalTokenValidator {
    private final String internalToken;

    public InternalTokenValidator(String internalToken) { this.internalToken = internalToken; }

    public void requireValidToken(String authorization) {
        // Bearer token validation, throws 401/403 on failure
        // timing-safe comparison via MessageDigest.isEqual()
    }
}

// Auto-configured by:
// urr-common/src/main/java/guru/urr/common/config/InternalTokenValidatorAutoConfiguration.java
@Configuration
@ConditionalOnProperty(name = "INTERNAL_API_TOKEN")
public class InternalTokenValidatorAutoConfiguration {
    @Bean
    public InternalTokenValidator internalTokenValidator(@Value("${INTERNAL_API_TOKEN}") String token) {
...
| 서비스 | 파일 위치 | 비고 |
|--------|-----------|------|
| **urr-common** (6개 서비스 공용) | `urr-common/src/main/java/guru/urr/common/security/InternalTokenValidator.java` | `@ConditionalOnProperty`으로 자동 등록 |
| **auth-service** (자체 버전) | `auth-service/src/main/java/guru/urr/authservice/security/InternalTokenValidator.java` | boolean 리턴, X-Internal-Token 이중 검증 |
