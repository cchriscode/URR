...
- **Gradle** (각 서비스 독립 프로젝트, 개별 `build.gradle` + 공유 라이브러리 `urr-common` Composite Build)
- 각 서비스에 `settings.gradle`이 별도 존재 (6개 서비스가 `includeBuild '../urr-common'`으로 공유 라이브러리 참조)
- **urr-common** (`services-spring/urr-common/`): `java-library` 플러그인. 6개 서비스(catalog, community, payment, queue, stats, ticket)가 의존하며, `@SpringBootApplication(scanBasePackages = {"guru.urr.<service>", "guru.urr.common"})`으로 빈 스캔
  - `JwtTokenParser` (`@Component`) — X-User-* 헤더에서 사용자 컨텍스트 추출
  - `GlobalExceptionHandler` (`@RestControllerAdvice`) — ResponseStatusException, MethodArgumentNotValidException, 범용 Exception 처리
  - `InternalTokenValidatorAutoConfiguration` (`@Configuration`, `@ConditionalOnProperty(name = "INTERNAL_API_TOKEN")`) — 서비스 간 내부 토큰 검증 빈 자동 등록
  - `DataSourceRoutingConfig` (`@Configuration`, `@ConditionalOnProperty(name = "spring.datasource.url")`) — Primary/Replica 읽기/쓰기 분리 라우팅
  - `ReadWriteRoutingDataSource` — `AbstractRoutingDataSource` 확장, 트랜잭션 readOnly 플래그 기반 라우팅
  - `AuthUser` (record) — userId, email, role 값 객체
...
| catalog-service | 2개 (V1~V2) | catalog core schema, ticket_types 인덱스 |
...
- 검증 측: `InternalTokenValidator.requireValidToken(authorization)` 호출
  - **urr-common 중앙화**: `services-spring/urr-common/src/main/java/guru/urr/common/security/InternalTokenValidator.java` — `@ConditionalOnProperty(name = "INTERNAL_API_TOKEN")`로 자동 등록 (`InternalTokenValidatorAutoConfiguration.java`)
  - catalog, ticket, payment, queue, stats, community 6개 서비스가 urr-common 버전 사용
  - auth-service: 자체 `InternalTokenValidator` (`services-spring/auth-service/src/main/java/guru/urr/authservice/security/InternalTokenValidator.java`) — `isValid()` boolean 리턴 방식 (X-Internal-Token 헤더 + Authorization Bearer 이중 검증)
  - timing-safe 비교: `MessageDigest.isEqual()` 사용 (`InternalTokenValidator.java`)
...
- **urr-common 중앙화**: `services-spring/urr-common/src/main/java/guru/urr/common/config/InternalTokenValidatorAutoConfiguration.java` — `@ConditionalOnProperty(name = "INTERNAL_API_TOKEN")`로 6개 서비스에 자동 등록
- auth-service는 자체 구현 사용 (boolean 리턴 + X-Internal-Token 이중 검증): `services-spring/auth-service/src/main/java/guru/urr/authservice/security/InternalTokenValidator.java`
...
### 10-1. 예매→결제→재고→통계 처리 방식

| 단계 | 방식 | 설명 |
|------|------|------|
| 좌석 선택 → 예매 생성 | **동기** (REST) | ticket-service 내부, Redis Lua + DB @Transactional |
| 예매 생성 → 통계 반영 | **비동기** (Kafka) | `reservation-events` → stats-service |
| ticket → payment 검증 요청 | **동기** (REST + CircuitBreaker) | `TicketInternalClient.validateReservation()` |
| payment 결과 → ticket 확정 | **비동기** (Kafka) | `payment-events` → `PaymentEventConsumer` |
| payment 결과 → 통계 | **비동기** (Kafka) | `payment-events` → stats-service |
| 좌석 잠금/해제 | **동기** (Redis) | ticket-service 내 Redis Lua 스크립트 |
| 좌석 생성/삭제 | **동기** (REST + CircuitBreaker) | catalog → ticket `TicketInternalClient` |

### 10-2. 좌석 상태 전이 (Seat Status Lifecycle)

```
예매 시:    seats.status = 'available' → 'locked'     (동기, Redis + DB)
결제 후:    seats.status = 'locked'   → 'reserved'    (비동기, Kafka Consumer)
환불 시:    seats.status = 'reserved' → 'available'    (비동기, Kafka Consumer)
만료 시:    seats.status = 'locked'   → 'available'    (스케줄러, 30초마다)
```
...
```
[사용자] → 예매 요청 (동기 REST)
  → ticket-service: 좌석 잠금 (동기 Redis Lua + Fencing Token)
  → ticket-service: 예매 생성 status='pending' (동기 DB @Transactional)
  → HTTP 200 응답 { reservationId, expiresAt: 5분 }

[사용자] → 결제 요청 (동기 REST)
  → payment-service: ticket-service 검증 (동기 REST + CircuitBreaker)
  → payment-service: 결제 처리 (동기 DB @Transactional)
  → payment-service: ticket-service 확정 시도 (동기 REST)
     └─ 실패 시 → 예외 발생 → catch → Kafka 폴백
  → payment-service: Kafka "payment-events" 발행 (항상 실행)
  → HTTP 200 응답 { paymentId, status: "confirmed" }

[비동기] ticket-service Kafka Consumer:
  → 예매 confirmed 업데이트
  → 좌석 status='reserved'
  → Redis 잠금 정리
  → 멤버십 포인트 적립
...
핵심: **사용자가 기다리는 구간(좌석 잠금, 결제 요청)은 동기**, 결과 후처리(통계 집계, 예매 확정)는 **비동기 Kafka**.
...
| 메커니즘 | 구현 | 출처 |
|----------|------|------|
| DB 트랜잭션 | `@Transactional` — 예매/결제 핵심 메서드 전부 적용 | `ReservationService.java:55,391,462` |
| 좌석 잠금 | Redis Lua 스크립트 + Fencing Token | `SeatLockService.java:39-62` |
| 동시성 제어 | DB Optimistic Locking (VERSION 체크) | `ReservationService.java:122-132` |
| 멱등성 | `processed_events` 테이블로 중복 처리 방지 | `PaymentEventConsumer.java:60-65` |
| Circuit Breaker + Retry | Resilience4j (3회 재시도, 지수 백오프) — 전체 내부 REST 호출 | `TicketInternalClient.java` |
| 만료 정리 | 30초마다 만료 예매 정리 스케줄러 | `ReservationCleanupScheduler.java:35` |
| 결제 보정 | 5분마다 불일치 예매 재확인 스케줄러 | `PaymentReconciliationScheduler.java:47` |
| DLQ (Dead Letter Queue) | Kafka Consumer 실패 → 3회 재시도 → `{topic}-dlq` 토픽 이동 | `KafkaConfig.java` (ticket/stats 양쪽) |

### 11-2. 장애 시나리오별 대응 현황

| 시나리오 | 대응 | 상태 |
|----------|------|------|
| 좌석 잠금 후 DB INSERT 실패 | `@Transactional` 롤백 + TTL(7분) 자동 만료 | SAFE |
| 결제 확정 후 ticket-service 다운 | CircuitBreaker fallback → 예외 → catch → Kafka 폴백 발행 | SAFE |
| Kafka Consumer 처리 실패 | DefaultErrorHandler 3회 재시도 (1s, 2s, 4s) → DLQ | SAFE |
| Kafka 자체 장애 | PaymentReconciliationScheduler가 5분 후 재시도 | SAFE |
| 결제 + Kafka 동시 장애 | DLQ + 보정 스케줄러 이중 안전망 | SAFE |
| 중복 결제 이벤트 (at-least-once) | `processed_events` 테이블 멱등성 체크 | SAFE |
| 동시 좌석 선점 (Race Condition) | Redis Lua 원자성 + DB Optimistic Lock | SAFE |
| 환불 처리 실패 | 예외 전파 → DLQ → 수동/자동 재처리 | SAFE |

### 11-3. Kafka DLQ 구성

ticket-service와 stats-service 양쪽에 동일한 패턴으로 구현:
...
| 서비스 | Consumer | DLQ 토픽 | 출처 |
|--------|----------|----------|------|
| ticket-service | `PaymentEventConsumer` | `payment-events-dlq` | `ticket-service/.../KafkaConfig.java` |
| stats-service | `StatsEventConsumer` (4개 리스너) | `{topic}-dlq` | `stats-service/.../KafkaConfig.java` |

### 11-4. TTL 타이밍 설계

| 항목 | 값 | 설정 |
|------|-----|------|
| 예매 만료 | 5분 | `ReservationService.java:136` |
| Redis 좌석 잠금 TTL | **7분 (420초)** | `application.yml: seat-lock.ttl-seconds: 420` |
| 버퍼 | 2분 | 결제 직전 잠금 만료 방지 |
| 정리 스케줄러 주기 | 30초 | `reservation.cleanup.interval-ms: 30000` |
| 보정 스케줄러 주기 | 5분 | `reservation.reconciliation.interval-ms: 300000` |

---

## 12. Saga 패턴 분석

### 12-1. 현재 구현: Choreography Saga

URR은 **명시적인 Saga Orchestrator가 없다**. 대신 이벤트 기반 Choreography Saga를 사용한다.

- 이벤트 DTO에 `sagaId` 필드가 존재하지만, **멱등성 키(deduplication key)로만 사용** (`PaymentEventConsumer.java:202-205`)
- Saga 상태 테이블(saga_state)은 없음
- 보상 트랜잭션(compensation)은 없음 — 대신 만료 스케줄러와 보정 스케줄러로 대체

### 12-2. 현재 구조 vs 정식 Saga

| 항목 | Orchestration Saga | URR 현재 |
|------|---|---|
| Saga 상태 관리 | `saga_state` 테이블 (STARTED→COMPLETED) | **없음** |
| 보상 트랜잭션 | 결제 실패 → 자동 좌석 해제 | 만료 스케줄러(30초)가 대체 |
| DLQ + 재시도 | 실패 메시지 재처리 | **구현 완료** (DefaultErrorHandler + DLQ) |
| 타임아웃 보상 | Saga timeout → 전체 롤백 | 5분 예매 만료 + 7분 Redis TTL |

### 12-3. 이 구조가 유효한 이유

1. **VWR이 트래픽을 제어**: 동시 결제 요청이 50-200건 수준으로 제한됨
2. **동기+비동기 이중 확인**: 동기 REST 성공 시 즉시 완료, 실패 시 Kafka가 보완
3. **보정 스케줄러**: Kafka 장애 시에도 5분 후 자동 복구
4. **DLQ**: Consumer 실패 메시지 영구 유실 방지

---

## 13. CQRS 확장 전략 (향후)

### 13-1. 현재 REST 의존 관계 중 CQRS 전환 후보

MSA에서 서비스 스케일아웃 시 동기 REST 호출이 과부하를 유발할 수 있다. CQRS(Command Query Responsibility Segregation) 패턴으로 읽기 요청을 로컬 DB 복사본으로 전환하면 서비스 간 결합도를 줄일 수 있다.

| 호출 | 현재 방식 | CQRS 전환 시 |
|------|-----------|-------------|
| catalog → auth (사용자 조회) | 동기 REST | auth가 `user-events` 발행 → catalog의 `users_cache` 테이블에 동기화 |
| queue → ticket (이벤트 정보) | 동기 REST | ticket이 `event-info-events` 발행 → queue의 `events_cache`에 동기화 |
| stats → (전체) | **이미 CQRS** | Kafka 이벤트를 받아 자기 DB에 통계 집계 |

### 13-2. CQRS 전환 기준

현재 규모(VWR 제어 하 50-200 동시 요청)에서는 동기 REST로 충분하다. 다음 조건 발생 시 CQRS 전환을 고려:

- catalog-service 스케일아웃 시 auth-service에 과부하 발생
- queue-service 스케일아웃 시 ticket-service에 과부하 발생
- 서비스 간 REST 호출 latency가 P99 500ms 초과

---

## 14. 15K 버스트 코드 준비도 평가

15,000명 동시 접속 시나리오에서 백엔드 코드 레벨의 적정성을 평가했다. VWR Two-Tier 구조에 의해 실제 백엔드 부하는 50-200명으로 제어되므로, 그에 맞는 코드 품질을 점검한다.

### 14-1. catalog-service 인덱스 누락 (수정 완료)

`V1__catalog_core_schema.sql`에서 `ticket_types` 테이블의 `event_id` FK에 인덱스가 없었다. PostgreSQL은 FK 컬럼에 자동 인덱스를 생성하지 않으므로, 이벤트 상세 조회 시 full table scan이 발생한다.
...
- 파일: `services-spring/catalog-service/src/main/resources/db/migration/V2__add_ticket_types_index.sql`

### 14-2. Kafka Producer `acks: all` 설정 검증

| 서비스 | acks | retries | 설정 위치 |
|--------|------|---------|-----------|
| payment-service | `all` (명시) | 3 | `application.yml:6-12` |
| ticket-service | `all` (명시) | Spring 기본값 | `application.yml:6-10` |
| stats-service | `all` (Kafka 3.0+ 기본값) | — | `KafkaConfig.java` (DLQ 전용 Producer) |

`acks: all`은 결제 이벤트와 예약 이벤트의 무결성을 보장하는 필수 설정이다. VWR로 실제 부하가 50-200명이므로 추가 지연(~5ms)은 무시 가능하다. 변경 불필요.

### 14-3. stats-service 단건 처리 적정성 검증

stats-service는 `@KafkaListener`로 메시지를 1건씩 수신하여 UPSERT 1-2회를 실행한다.

실제 부하: 200명 동시 예매 기준 ~7 events/sec × UPSERT ~2ms = 14ms/sec (CPU 1.4%). 배치 전환 시 `BatchListenerFailedException` 처리, deduplication 로직 변경 등 복잡도가 크게 증가하지만 절감 효과(14ms → 5ms)가 미미하다. 현재 단건 처리가 VWR 보호 하의 부하 수준에 적합하다.

### 14-4. 인프라 레벨 수정 사항 (상세: `04_인프라_분석.md` 섹션 8 참조)

| 수정 항목 | 이전 | 이후 | 심각도 |
|-----------|------|------|--------|
| Lambda 동시실행 수 | 100 | 500 | CRITICAL |
| queue-service CPU request | 100m | 250m | CRITICAL |
| ticket_types 인덱스 | 없음 | `idx_ticket_types_event_id` | CRITICAL |
| ISR (Next.js) | — | 이미 `revalidate: 60` 적용 확인 | 오탐 |
| Kafka acks: all | all | 유지 (변경 금지) | 정상 |
| stats 배치 처리 | 단건 | 유지 (변경 불필요) | 불필요 |

---

## 15. RDS Read Replica 라우팅 구현

### 15-1. 배경

효율성/비용 감사 결과, RDS Read Replica가 프로비저닝되어 있으나 어떤 서비스도 활용하지 않아 월 ~$90의 비용이 낭비되고 있었다. 읽기 비중이 높은 3개 서비스(catalog, stats, community)에 **AbstractRoutingDataSource** 패턴을 적용하여 Read Replica를 활용하도록 구현했다.

### 15-2. 대상 서비스 및 읽기 메서드

| 서비스 | 읽기 전용 클래스/메서드 | 적용 방식 | 비고 |
|--------|----------------------|-----------|------|
| **catalog-service** | `EventReadService` 전체 (7개 메서드) | 클래스 레벨 `@Transactional(readOnly = true)` | listEvents, getEventDetail, getTicketsByEvent, getTicketAvailability, getSeatLayouts, getSeatsByEvent, getEventQueueInfo |
| **stats-service** | `StatsQueryService` 전체 (14개 메서드) | 클래스 레벨 `@Transactional(readOnly = true)` | overview, daily, events, eventById, payments, revenue, users, hourlyTraffic, conversion, cancellations, realtime, seatPreferences, userBehavior, performance |
| **community-service** | `PostService.list()` | 메서드 레벨 `@Transactional(readOnly = true)` | `detail()`은 `UPDATE views = views + 1`이 있어 **제외** |
| **community-service** | `CommentService.listByPost()` | 메서드 레벨 `@Transactional(readOnly = true)` | — |
| **community-service** | `NewsService.list()` | 메서드 레벨 `@Transactional(readOnly = true)` | `detail()`은 `UPDATE views = views + 1`이 있어 **제외** |

### 15-3. 구현 구조

**urr-common에 공유 설정 클래스 2개를 중앙화** (개별 서비스에서 삭제됨):

```
services-spring/urr-common/src/main/java/guru/urr/common/config/
├── ReadWriteRoutingDataSource.java   ← AbstractRoutingDataSource 확장
└── DataSourceRoutingConfig.java      ← @ConditionalOnProperty(name = "spring.datasource.url") + Primary/Replica HikariDataSource + 라우팅 빈
```

`DataSourceRoutingConfig`는 `@ConditionalOnProperty(name = "spring.datasource.url")`로 조건부 활성화되어, DB가 없는 서비스(queue-service 등)에서는 자동으로 비활성화된다.
...
1. `@Transactional(readOnly = true)` → Spring이 `TransactionSynchronizationManager`에 읽기 전용 플래그 설정
2. `ReadWriteRoutingDataSource`가 플래그를 감지하여 `"replica"` 키 반환
3. `DataSourceRoutingConfig`에서 `"replica"` 키에 매핑된 Read Replica HikariDataSource로 커넥션 제공

**폴백 동작:**
- `replica-url`이 비어있으면 primary URL로 폴백 → 로컬/dev 환경에서 기존과 동일하게 동작
- `@Primary` DataSource 빈이 Spring Boot의 `DataSourceAutoConfiguration`을 자동으로 대체 (exclude 불필요)
- Flyway 마이그레이션은 트랜잭션 컨텍스트 없이 실행되므로 항상 primary로 라우팅

### 15-4. 변경 파일 목록

| # | 파일 | 작업 |
|---|------|------|
| 1 | `urr-common/config/ReadWriteRoutingDataSource.java` | 신규 (urr-common에 중앙화) |
| 2 | `urr-common/config/DataSourceRoutingConfig.java` | 신규 (`@ConditionalOnProperty(name = "spring.datasource.url")`) |
| 3 | `catalog-service/application.yml` | `replica-url` 프로퍼티 추가 |
| 4 | `catalog-service/EventReadService.java` | 클래스 레벨 `@Transactional(readOnly = true)` |
| 5 | `stats-service/application.yml` | `replica-url` 프로퍼티 추가 |
| 6 | `stats-service/StatsQueryService.java` | 클래스 레벨 `@Transactional(readOnly = true)` |
| 7 | `community-service/application.yml` | `replica-url` 프로퍼티 추가 |
| 8 | `community-service/PostService.java` | `list()` 메서드 레벨 어노테이션 |
| 9 | `community-service/CommentService.java` | `listByPost()` 메서드 레벨 어노테이션 |
| 10 | `community-service/NewsService.java` | `list()` 메서드 레벨 어노테이션 |

### 15-5. 프로덕션 배포 설정

K8s 환경변수에 Read Replica 엔드포인트 추가:

```yaml
CATALOG_DB_REPLICA_URL: jdbc:postgresql://<rds-replica-endpoint>:5432/catalog_db
STATS_DB_REPLICA_URL: jdbc:postgresql://<rds-replica-endpoint>:5432/stats_db
COMMUNITY_DB_REPLICA_URL: jdbc:postgresql://<rds-replica-endpoint>:5432/community_db
```

### 15-6. 기대 효과

- **Primary RDS 부하 분산**: 읽기 쿼리(전체의 ~70%)가 Replica로 분산되어 쓰기 성능 향상
- **비용 효율**: 이미 프로비저닝된 Read Replica($90/월)를 활용하여 추가 비용 없이 성능 개선
- **수평 확장 기반**: 트래픽 증가 시 Read Replica 추가만으로 읽기 성능 확장 가능
