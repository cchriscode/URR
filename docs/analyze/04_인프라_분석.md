...
**Staging 환경 (16개 모듈)** - `terraform/environments/staging/main.tf`
...
| 14 | lambda_vwr | `modules/lambda-vwr` | 233-253 |
| 15 | lambda_worker | `modules/lambda-worker` | 259-298 |
| 16 | monitoring | `modules/monitoring` | 304-309 |

**Prod 환경 (20개 모듈)** - `terraform/environments/prod/main.tf`

Staging의 16개 모듈에 추가로 다음 4개 모듈이 포함된다:
...
> **urr-common 경로 트리거**: 6개 의존 서비스(catalog, community, payment, queue, stats, ticket)의 CI/CD 워크플로우는 `services-spring/urr-common/**` 경로를 트리거에 포함한다. urr-common이 변경되면 6개 서비스가 자동 재빌드된다. auth-service와 gateway-service는 urr-common에 의존하지 않으므로 이 트리거가 없다.
...
- **Docker build context**: `services-spring/` 디렉토리 (라인 171). 레포 루트(`.`)가 아니므로, 모든 Dockerfile의 경로는 `services-spring/` 기준 상대경로를 사용한다.
- **urr-common 복사**: 6개 서비스(catalog, community, payment, queue, stats, ticket)의 Dockerfile은 빌드 첫 단계에서 `COPY urr-common /workspace/urr-common`으로 공유 라이브러리를 복사한다. auth-service와 gateway-service는 urr-common을 사용하지 않으므로 이 단계가 없다.
...
- **`.dockerignore`**: `services-spring/.dockerignore`가 빌드 아티팩트(`**/build/`, `**/.gradle/` 등)를 제외하여 빌드 컨텍스트 크기를 최소화한다.
...
## 9. RDS Read Replica 활용

### 9-1. 문제점

Terraform에서 Read Replica를 프로비저닝하고 있으나, 백엔드 서비스에서 활용하지 않아 월 ~$90의 비용이 낭비되고 있었다.

**기존 구성 (Terraform):**
- Prod: `read_replicas = 1` → Read Replica 1대 생성 (`terraform/modules/rds/main.tf`)
- Staging: `read_replicas = 0` → Replica 없음
- 각 서비스 `application.yml`에 단일 `spring.datasource.url`만 설정 → 모든 쿼리가 Primary로

### 9-2. 해결 방안

읽기 비중이 높은 3개 서비스에 **Spring AbstractRoutingDataSource** 패턴 적용:

| 서비스 | 읽기 메서드 수 | DB 접근 방식 |
|--------|-------------|-------------|
| catalog-service | 7개 (EventReadService 전체) | JDBC |
| stats-service | 14개 (StatsQueryService 전체) | JPA + JdbcTemplate |
| community-service | 3개 (list/listByPost 메서드) | JDBC |

### 9-3. 인프라 측 변경

각 서비스의 `application.yml`에 `replica-url` 프로퍼티를 추가하고, K8s 배포 시 환경변수로 Read Replica 엔드포인트를 주입하는 구조:

```yaml
spring:
  datasource:
    url: ${DB_URL}           # Primary (읽기+쓰기)
    replica-url: ${DB_REPLICA_URL:}  # Replica (읽기 전용, 미설정 시 Primary 폴백)
```

**폴백 설계**: `replica-url`이 비어있으면 primary URL로 자동 폴백하여, 로컬 개발 환경이나 Staging(Replica 없음)에서도 정상 동작한다.

### 9-4. 기대 효과

| 항목 | Before | After |
|------|--------|-------|
| Read Replica 비용 | $90/월 낭비 | $90/월 활용 |
| Primary RDS 읽기 부하 | 100% | ~30% (읽기의 ~70% 분산) |
| 수평 확장 | 불가 | Replica 추가로 읽기 확장 가능 |
