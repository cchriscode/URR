...
서버가 바쁘면 사용자는 계속 기다려야 한다.

B가 느리거나 죽으면, A도 같이 멈춘다(문서에 나오는 문제).

0-2. 비동기 처리
번호표(대기표)를 뽑고,
“나중에 호출하면 움직이자” 식으로 동작

요청을 “메시지”로 남겨두고,
실제 처리는 나중에 다른 애(Consumer)가 해도 된다.

알아야 할 포인트

“지금 바로 처리하지 않고, 일단 할 일 목록에 넣어두는 처리 방식이 있다”

그 할 일 목록이 바로 Queue / Topic으로 구현되는 것

---

2. 메시지 / 이벤트라는 개념
2-1. 메시지(message)
“해야 할 일” 또는 “일어난 일”을 한 덩어리로 묶은 것
...
메시지/이벤트는 그냥 데이터 한 덩어리일 뿐이고,

이걸 잘 모아서, 차례대로 또는 여러 곳에 뿌려주는 시스템이 SQS/Kafka 같은 메시징 시스템이다.
...
▷. Queue와 Topic 비유를 이해하기 위한 수준
문서에 이미 비유가 좋게 들어가 있어서, 이 정도만 알면 된다.

4-1. Queue = “일감 대기열”
고객센터 전화 대기열, 병원 진료 대기 순번 같은 것

한 사람(Consumer)이 한 건 처리하고 나면 그 일은 끝.

포인트

“한 메시지는 보통 한 소비자가 처리하고 끝난다”

4-2. Topic = “공지 게시판”
회사 공지방: 공지 1개를 여러 팀이 각자 읽고 자기 할 일 한다.

같은 메시지가 여러 소비자 그룹에 전달될 수 있다.

포인트

“같은 메시지를 여러 서비스가 각자 처리할 수 있다”

▷. Producer / Consumer / Broker 개념
5-1. Producer (발행자)
메시지를 “써 넣는” 주체

예: 결제 서비스가 “결제 완료” 이벤트를 Kafka에 발행

5-2. Consumer (소비자)
메시지를 “읽어서 처리하는” 주체

예: 티켓 서비스가 “결제 완료” 이벤트를 보고 예매를 확정

5-3. Broker (중간 보관소)
SQS, Kafka 처럼 메시지를 받아서 보관하고, 나중에 소비자에게 전달해주는 시스템

알아야 할 포인트

“A 서비스 ↔ B 서비스가 직접 통신하는 대신,
A → Broker(SQS/Kafka) → B 구조로 느슨하게 이어진다”

▷ 오류/장애 상황에 대한 기본 감각
문서에 ACK, Retry, DLQ, Idempotency가 나와 있어서,
이 네 가지를 이해하려면 다음 정도의 감각이 필요하다.

6-1. ACK (처리 성공 알림)
“나 이 메시지 처리 다 했어”라는 확인 신호

없어지면 재처리 위험이 있다.

6-2. Retry (재시도)
일시적으로 DB가 바쁘거나 네트워크 문제가 나면
“다시 한 번 해보자”는 로직이 필요하다.

6-3. DLQ (Dead Letter Queue)
계속 실패하는 메시지(예: 잘못된 데이터, 버그)를 따로 모으는 쓰레기통 큐

나중에 운영자가 보고 “왜 이건 계속 실패하지?”를 분석하기 위한 용도

6-4. Idempotency (멱등성)
같은 메시지가 2번 와도, 결과는 1번 처리한 것과 같게 만드는 성질

예: “이 결제는 이미 처리됨”을 DB에 기록해 두고,
두 번째 요청은 무시

알아야 할 포인트

네트워크와 분산 시스템에서는 중복/실패/지연이 “당연히” 일어난다.

그래서 재시도, 실패 모으기(DLQ), 중복 방어(멱등성) 개념이 필요하다.

▷ AWS SQS와 Kafka를 이해하기 위한 최소 사전 지식
7-1. “클라우드 서비스” 개념
AWS는 “우리가 직접 서버/큐/DB를 구축하지 않고, 빌려쓰는 플랫폼”

SQS는 “AWS가 미리 만들어놓은 큐 서비스”

Kafka는 보통 직접 운영하는 오픈소스 시스템 (Managed Kafka도 있지만, 기본 마인드는 “우리가 클러스터 띄운다”에 가깝다)

7-2. SQS의 Standard vs FIFO 감각
Standard: 빨리빨리, 순서/중복 조금은 틀어져도 OK

FIFO: 순서와 중복 제어에 신경쓰는 모드

여기서 MessageGroupId, MessageDeduplicationId 같은 키로
“같은 그룹끼리 순서, 같은 메시지 중복 방지”를 한다는 정도만 알면 충분.

7-3. Kafka에서 Topic/Partition/Consumer Group
Topic: “이벤트 종류별 채널 이름” (payment-events, reservation-events 등)

Partition: 같은 토픽을 여러 조각으로 나눈 것 (병렬 처리, 순서 보장 단위)

Consumer Group: “같은 역할을 하는 소비자들의 그룹”

같은 그룹 안에서는 일을 나눠서 처리(병렬)

그룹이 다르면, 같은 이벤트를 각 그룹이 따로 한 번씩 소비 (팬아웃)
