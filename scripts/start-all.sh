...
echo -e "${CYAN}[1/3] Starting databases and Redis ...${RESET}"
...
DB_PORTS=(5433 5434 5435 5436)
DB_NAMES=(auth-db ticket-db payment-db stats-db)
...
for entry in "${SERVICES[@]}"; do
  ...
  while [ $RETRIES -lt $MAX_RETRIES ]; do
    if curl -sf --max-time 2 "http://localhost:$SVC_PORT/actuator/health" >/dev/null 2>&1; then
